...
export const configs = {
    service: "student",
    environment: process.env.LAB_ENVIRONMENT || "dev",
    app: {
        prefix: "/api/v1",
        host: process.env.LAB_HOST_NAME || "0.0.0.0",
        port: process.env.LAB_PORT_NUMBER || "3000",
    },
    mongo: {
        mode: process.env.LAB_MONGO_MODE || "STANDALONE",
        replicaName: process.env.LAB_MONGO_REPLICA_NAME || "rs0",
        addresses: process.env.LAB_MONGO_ADDRESSES || "127.0.0.1:27001",
        username: process.env.LAB_MONGO_USERNAME || "root",
        password: process.env.LAB_MONGO_PASSWORD || "",
        authSource: process.env.LAB_MONGO_AUTHOR_SOURCE || "admin",
        dbName: process.env.LAB_MONGO_DB_NAME || "tenant",
        templateUri:
